...
let input = readLines "input03.txt"
...
let parseLine line = Array.ofSeq line |> Array.map transform

let forest: bool [] list =
  input |> List.ofSeq |> List.map parseLine
...
let rec step (xcord: int, ycord: int) (xs: int, ys: int) (input: bool [] list) trees =
  ...
  | row :: tail ->
      let tree =
        if (Array.get row xcord) then trees + 1L else trees

      let column: int = (xcord + xs) % width

      if (List.length tail >= ys)
      then step (column, ycord + ys) (xs, ys) (List.skip (ys - 1) tail) tree
      else tree
